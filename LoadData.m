function data = LoadData(sequenceName, frameIDs)
    ...
    % read intrinsic parameters,use strrep function to make urlread function work
    data.K = reshape(readValuesFromTxt(strrep(fullfile(SUN3Dpath,sequenceName,'intrinsics.txt'),'\','/')),3,3)';
    
    % file list
    imageFiles = dirSmart(strrep(fullfile(SUN3Dpath,sequenceName,'image/'),'\','/'),'jpg');
    depthFiles = dirSmart(strrep(fullfile(SUN3Dpath,sequenceName,'depth/'),'\','/'),'png');
    extrinsicsFiles = dirSmart(strrep(fullfile(SUN3Dpath,sequenceName,'extrinsics/'),'\','/'),'txt');
    ...
    % read annotation:
    data.annotation = annotationRead(strrep(fullfile(SUN3Dpath,sequenceName,'annotation/index.json'),'\','/'));
    ...
    for frameID=frameIDs
        cnt = cnt + 1;
        data.image{cnt} = strrep(fullfile(fullfile(SUN3Dpath,sequenceName,'image',imageFiles(frameID).name)),'\','/');
        data.depth{cnt} = strrep(fullfile(fullfile(SUN3Dpath,sequenceName,'depth',depthFiles(IDimage2depth(frameID)).name)),'\','/');
    end
    
    try
        % read the latest version of extrinsic parameters for cameras
        data.extrinsicsC2W = permute(reshape(readValuesFromTxt(strrep(fullfile(SUN3Dpath,sequenceName,'extrinsics',extrinsicsFiles(end).name),'\','/')),4,3,[]),[2 1 3]);
    catch
        data.extrinsicsC2W = [];
    end
