function [ data ] = loadSUN3D_Mysequence( sequenceName, frameIDs, SUN3Dpath )
...
dir_data = [SUN3Dpath sequenceName];
dir_image = [dir_data 'image/'];
dir_depth = [dir_data 'depth_sun/'];
file_data_associate = [dir_data 'associate.txt'];
file_handler = fopen(file_data_associate,'r')
line = fgetl(file_handler);
size_data = 0;
num_test =100;
while ischar(line)
    size_data = size_data+1;
    line_split = textscan(line,'%f %s %f %s');
    data.image{size_data} = [dir_image char(line_split{2})];
    data.depth{size_data} = [dir_depth char(line_split{4})];
    line = fgetl(file_handler);
%     if size_data > num_test
%         break;
%     end
end

frameCount = length(data.image);
 % read annotation:
    try
